...
#   usage: newGbExecute.pl <properties-file>
#=============================================================================
...
my $maxToolScore          = $cfg->getProperty( 'max.score.tools', 10 );
...
my $styleLogRelative   = "style.log";
my $styleLog	       = "$log_dir/$styleLogRelative";
...
if ( $can_proceed )
{
    ...
    $runtimeScoreWithoutCoverage =
        $maxCorrectnessScore
        * ( ( $instructorTestsRun - $instructorTestsFailed )/ $instructorTestsRun );
	...
	$staticScore  = (`tail -4 ${working_dir}/styleSummary.txt | grep -o [0-9] | tail -1`);
        (`mv styleSummary.txt $log_dir`);
	# (`rm ${working_dir}/styleSummary.txt`);

	$stylePercent = $staticScore * 10;
...
if ( $can_proceed )
{
	open my $fh2, '<', "${log_dir}/style.txt";
	my @lines = <$fh2>;
    close $fh2;

    my $feedbackGenerator = new Web_CAT::FeedbackGenerator( $styleLog );
	    $feedbackGenerator->startFeedbackSection( "Checking your Style" );
		$feedbackGenerator->print(<<EOF);
<pre>
@lines
</pre>
EOF
 		$feedbackGenerator->print( "<p><b>Style Coverage: </b>" );
        $feedbackGenerator->print(
                "<font color=\"#ee00bb\">$stylePercent%</font>" );

		$feedbackGenerator->endFeedbackSection();
    	$feedbackGenerator->close;
  		$reportCount++;
            	$cfg->setProperty( "report${reportCount}.file", $styleLogRelative );
            	$cfg->setProperty( "report${reportCount}.mimeType", "text/html" );
}
...
#=============================================================================
# Color the Style Checking
#=============================================================================
# sub color
# {
#     use strict;
#     my @fileNames = (`ls $log_dir/html/`);
#     chdir("${log_dir}/html/");

#     foreach my $fn (@fileNames)
#     {
#         $fn =~ s/\s+$// ;
#         my @e = readtoVar("$log_dir/style.txt");
#         readtoVar($fn)
#     }
# }
...
if ( $can_proceed )
{
	...
	$scoreToTenths = ($staticScore + $scoreToTenths);
...
EOF
	$feedbackGenerator->print( <<EOF );
<tr><td><b>Style Coverage:</b></td>
<td class="n">$stylePercent%</td>
<td>(how your code style checks out)</td></tr>
</table>
<p>Your Correctness/Testing score is calculated this way:</p>
<p>Score = $maxCorrectnessScore% * $studentCasesPercent%
EOF
    $feedbackGenerator->print( <<EOF );
* $instructorCasesPercent + ($stylePercent%) = $scoreToTenths</p>
<p>Note that full-precision percentages are used to calculate
your score.</p>
EOF
    $feedbackGenerator->endFeedbackSection();
    $feedbackGenerator->close;
